...
def trainer(dataset):
    ...
    for epoch in range(args.max_epoch):
        ...
        if epoch % args.update_interval == 0:
            # update_interval
            A_hat, z, x_hat, B_hat, _, _, Q = model(data, B, adj, M)
        ...
        A_hat, z, x_hat, B_hat, x_hat2, B_hat2, q = model(data, B, adj, M)
        ...
        loss = L_r + args.beta * kl_loss

        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    #         with torch.no_grad():
    #             _, z, _, _, _ = model(data,B, adj, M)
    #             k_means = KMeans(n_clusters=args.n_clusters, n_init=20).fit(
    #             z.data.cpu().numpy()
    #            )
    #             acc, nmi, ari, f1 = eva(y, k_means.labels_, epoch)
    #             if acc > max_acc:
    #                 max_acc = acc
    #                 result = [epoch, acc, nmi, ari, f1]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lambda1", type=float, default=0.5, help="lambda for the reconstruct loss.")
    parser.add_argument("--beta", type=float, default=10, help="lambda for the KL loss.")
    ...
    # SiteSeer数据集 3312个节点，4723条边。标签共7个类别。数据集的特征维度是3703维。
    ...
    args.pretrain_path = f"./pretrain/pre_ddgae_{args.name}.pkl"
